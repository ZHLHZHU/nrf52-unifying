...
┌─────────────────────────────────────────────────────────────┐
│                        上位机 (Host)                          │
│   tools/unifying.py  (CLI)        tools/cdc_dfu.py (OTA)      │
└───────────────────────────┬─────────────────────────────────┘
                            │ USB-CDC (ASCII 行协议 / 二进制镜像)
┌───────────────────────────▼─────────────────────────────────┐
│                    nRF52840 固件 (app/)                       │
│                                                               │
│  main.rs  ── 单 embassy 执行器,join(usb_fut, serial_fut)     │
│   ├─ serial_fut: 命令分发 + 后台 keep-alive (select)          │
│   ├─ OTA 协议 (PING/INFO/WRITE/BOOT/...)                      │
│   └─ Unifying 协议 (UPAIR/UCONNECT/UTYPE/UKEY/...)            │
│                                                               │
│  ┌────────────────────────────────────────────────────────┐ │
│  │            rust-unifying (no_std lib)                    │ │
│  │  UnifyingDevice<R,C,E>  ── 协议状态机/配对/加密键击      │ │
│  │    trait UnifyingRadio   trait Clock   trait AesEncryptor│ │
│  └──────┬──────────────────────┬──────────────┬────────────┘ │
│         │                      │              │              │
│   esb_radio.rs           unifying_hal.rs  unifying_hal.rs    │
│   EsbRadio               EmbassyClock     SwAesCtr           │
│   (片上 RADIO/ESB)       (embassy-time)   (aes+ctr)         │
│                                                               │
│  storage.rs ── 配对信息持久化到 STORAGE 分区                  │
│  keymap.rs  ── ASCII → HID 扫描码                            │
└───────────────────────────┬─────────────────────────────────┘
                            │ 2.4GHz ESB (nRF24L01+ 兼容)
┌───────────────────────────▼─────────────────────────────────┐
│         Logitech Unifying 接收器 (046d:c52b)                 │
└──────────────────────────────────────────────────────────────┘
```
...
```
对地址 [a0,a1,a2,a3,a4]:
  reversed = [a4,a3,a2,a1,a0]      # rf24.rs 先反转 5 字节
  prefix   = reversed[0].reverse_bits()           = a4 位反转
  base     = pack_be([reversed[1..5] 各字节位反转]) = [a3,a2,a1,a0]
```
...
```
1. 发送: txen → READY ─(short)→ START → [发包] → END ─(short)→ DISABLE
2. 等 DISABLED
3. 收 ACK: rxen → READY ─(short)→ START → [收ACK] → END ─(short)→ DISABLE
4. 等 END(或超时 ACK_WAIT_US=600μs)
5. 校验 CRC,提取 ACK payload(若有)
```
